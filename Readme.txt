...
23. 绘制商品详情页页面基本结构
    ...
    20.1 对照app可以发现，商品详情页面，是由卡片视图组成的，所以我们可以去 MUI 里面找相似的 卡片视图布局

    20.2 布局完之后，可以发现商品详情页分为三个部分，第一个是由一个轮播图组成，第二部分是商品贩卖信息，第三部分是商品参数区域

        20.2.1 我们先看看轮播图，我们一开始的时候就在首页有做过一次轮播图，所以我们可以直接拿来用，既然可以拿来用，那首先就得
        把轮播图弄成子组件的形式，所以我们单独把轮播图给弄出来，但是由于，没有图片，所以我们做成子组件的时候，是要把从服务器拿到
        的图片做成数组，然后把这个数组当做参数，传给轮播图，而且有时候我们从服务器获取到的图片叫src，不叫img，但是我们子组件那边
        是取 img，如果改了子组件那么其他地方也要改，这是行不通的，所以我们只能在当前页面改，就是把 src 改成 img，循环轮播图数组
        的每一项，为 item 添加 img 属性，把 img 属性值改成 src 属性值就可以了

        20.2.2 我们做了商品详情页的轮播图，但是由于首页和商品详情页图片不一样，所以商品详情页的图片如果也设置成宽高100%，看起来就很奇怪，
        但是首页设置宽高100%就很正常，所以我们要做的就是，让首页图片宽高 100%，商品详情页的图片 高度100% 宽度自适应，这样会比较好。

        我们来分析一波：为什么商品详情页的轮播图那么难看
        1. 首页中的轮播图，它的宽和高都是采用了 100% 的宽度。
        2. 在商品详情页中，轮播图的图片，如果也使用了 100% 的宽度的话，那么图片就会不好看了
        3. 所以我们期望在商品详情页中的轮播图是高度为 100% 但是宽度为 自适应，而首页则都是 100%
        4. 所以经过分析，得到的问题的原因：首页中的轮播图 和 商品详情页中的轮播图，分歧点就是 宽度到底是 自适应 还是 100%
        5. 既然这两个 轮播图 ，其他方面都是没有冲突的，只是宽度有分歧，那么 我们可以定义 一个 属性，让使用 轮播图的调用者，
           手动指定是否为 100% 的宽度
        6. 两边的轮播图都设置了一个属性值 isfull 那么在 轮播图子组件中 就要接收 这个 属性值，然后 我们默认 轮播图不是 宽度
           为 100% ，如果要百分之百，那么我们自己手动给它加上一个类设置成百分百，所以这就涉及到了 用 vue 去操纵 元素的样式

    20.3 使用编程式导航实现图文介绍和商品评论页面的跳转
        this.$router.push({name:"组件对应的名字",params:{传过去的参数}})

    20.4 点击加入购物车按钮实现一个小球从商品件数那里跳到下面的导航栏的购物车上
        20.4.1 先画个小球
        20.4.2 根据定位，把小球移动到商品件数的那个位置，然后点击加入购物车按钮，实现小球的隐藏和显示
        20.4.3 接下来就是设置小球的位移，设置位移之前要用 transition 把小球包裹起来
               由于我们需要的是半场动画，就是只有去没有回，所以不能用以前的类，如 .leave-to-enter 啥，我们只能用 动画的钩子函数
               @before-enter  @enter  @after-enter 这三个
               @before-enter：刚刚开始的时候，摆正小球位置(0,0就是小球定位的位置)
               @enter：开始动画的时候 和 动画效果的绘制
               @after-enter：结束动画让小球隐藏
        20.4.4 出现的问题：由于我们把小球终点的位置写死了，所以不同分辨率手机位置会不一样

               小球动画优化思路：
               1. 先分析导致动画不准确的本质原因：我们把小球最终位移到的位置，已经局限在了某一分辨率下的某一点位置，滚动条未滚动的情况下；
               2. 只要分辨率和测试的时候不一样，或者滚动条有一定的滚动距离之后，问题就出现了；
               3. 因此，我们经过分析，得到结论：不能把最终位置的横纵坐标写死，而是应该根据不同情况，动态计算这个坐标值；
               4. 经过分析，得出解题思路：先得到徽标(小球最终要落在徽标的位置上)的横纵坐标，再得到小球的横纵坐标，然后让他们的y值求差，x
                  值也求差(徽标的坐标减去小球的坐标)，得到的结果就是横纵坐标要位移的距离
               5. 如何获取徽标和小球的位置：在DOM操作里面有一个属性：domObject.getBoundingClientRect(left,top,right,bottom)
                  这就是获取到的徽标和小球的位置.
               6. 给小球添加一个refs属性(相当于dom中的id)，然后用 getBoundingClientRect 获取到这个小球的位置
               7. 我们只是为了拿到 徽标 的坐标位置，所以我们可以采用 DOM 操作来拿到徽标的 id，本来是因为在vue中最好不要用dom操作
                  但是那是因为在获取数据上，或者渲染页面的时候，这时候不要用dom 操作，但是我们只是为了拿到 徽标的位置，很小的操作
                  所以可以用下DOM操作，这样会比较方便些。[在组件中，操作DOM的时候，拿到对应的ID跟父组件 子组件没关系，只要在当前页面中，可以拿到当前页面的任何元素]

        20.4.5 点击购物车的时候，我们如何要拿到选择的数量
          分析：1. 经过分析发现：购物车按钮属于 goodsinfo 组件页面的，而数字 是属于 numberbox 组件的
               2. 由于涉及到了父子组件的嵌套了，所以无法直接在 goodsinfo 页面中获取到选中的商品数量值
               3. 怎么解决这个问题：涉及到了 子组件向父组件传值了(事件调用机制)
               4. 事件调用机制本质：父组件向子组件传递方法，子组件调用这个方法，同时把数据当做参数传递给这个方法。
               5. 在父组件[goodsinfo]那边定义一个方法[getSelectedCount]通过事件调用机制[@getcount="getSelectedCount"]
                  传给子组件[goodsInfo_numberBox]
               6. 那么子组件什么时候把数据传递给父组件呢？
                  6.1 不管你点加号还是减号，每点一次，你都要把里面的数值传递给父组件一次，而且方框里面的数字你要改一次也要传一次，所以
                      总共你要绑定三个点击事件，这是方法一
                  6.2 方法二：使用 @change 事件，我不管你点加号还是减号，或者修改里面方框的值，我只要注意你的文本框里面的值改变了，就把这个值给传递给父组件
                  6.3 父组件接收子组件传递过来的值，然后保存到data中，到时候会有用

        20.4.6 设置数量控件的最大值，最小值肯定是1，不用设置，但是最大值要根据商品的库存量来设置，不能让最大值超过库存的量
               1. 库存量是在父组件[goodsinfo]中的，而数量控件是在子组件[numberBox]中的,所以这又是一个父组件向子组件传值的问题
                  可以通过:数据绑定来传值
               2. 这时候父组件应该把库存量传给子组件，子组件那边接收到库存量，但是因为由于库存量是存在服务器的，所以在获取数据的时候
                  是一个带 then 的前面是一个 promise 的异步操作，就是父组件传值给子组件的时候，可能 goodsinfo 还没获取到数据，这时候
                  传给他就是一个空对象里面的不存在的一个目标
               3. 因为我存的是假数据，所以不会出现这个问题，但是真正的项目是从服务器获取到的
               4. 问题是：我们不知道什么时候能够拿到 库存量 的数值，但是，总归有那么一刻，会得到一个真正的 库存量 的值。
